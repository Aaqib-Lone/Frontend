...
const Chatbot = (props) => {
    ...
    const handleSupport = async (support_flag) => {
        try {
            const res = await axios.post("https://work-u0a5.onrender.com/api/v1/support/addSupport", { support_flag: support_flag, socket_id: socket.id }, {
                headers: {
    ...
    const addSupportLog = async (review) => {
        try {
            const res = await axios.post("https://work-u0a5.onrender.com/api/v1/history/addHistory", { ...support_log, end_time: `${new Date(Date.now()).getHours()}:${new Date(Date.now()).getMinutes()}`, status: (review ? "Resolved" : "Not Resolved") }, {
                headers: {
                    Authorization: `Bearer ${user.token}`
                }
            });
            ...
            <div className='  fixed flex justify-center items-center bottom-[2rem] right-[2rem] flex-col z-50' >
                {vis == 1 && <div class="flex flex-col items-center justify-center h-[73vh] bg-gray-100 text-gray-800  w-[23rem] min-h-[30rem] rounded-xl">
                    ...
                    <div class="flex flex-col flex-grow w-full max-w-xl bg-white shadow-xl rounded-xl overflow-hidden">
                        <div className='W-[100%] min-h-[3rem] bg-black flex justify-end items-center'> <img src='https://1000logos.net/wp-content/uploads/2018/01/SBI-symbol.jpg' class='w-15 h-10' alt='hi'/>
                                        ...
                                        <div class="flex w-full mt-5 space-x-3 max-w-xs flex-col">
                                            ...
                                            <div>
                                                <div class="buttonss">
                                                    ...
                                                    <button id='2' className='buttons' onClick={(e) => {
                                                        handleSupport(e.target.id);
                                                    }}>Call</button>
                        ...
                        <div class="bg-black p-4 flex-row">
                ...
                <div className='flex w-[100%] justify-end m-[1rem]'>
                    <img src={BotImg} className=' flex items-center justify-center flex-shrink-0 h-[5rem] w-[5rem] rounded-full bg-black ' onClick={() => {
                        setvis(!vis)
                    }}/>
